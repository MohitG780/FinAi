...
(function () {
  ...
  function renderTemplateChips() {
    ...
    container.querySelectorAll('.template-chip').forEach(chip => {
      chip.addEventListener('click', () => {
        ...
        // Clean up UI if switching from upload mode
        state.isDocUploaded = false;
        $('file-ready-card').classList.add('hidden');
        $('paste-text-wrapper').classList.remove('hidden');
        $('input-label').textContent = 'Paste Financial Text';

        showToast('✏️', `Loaded: ${DATA.templates[idx]}`);
        
        // Auto-run analysis for a premium feel
        runAnalysis(DATA.templates[idx]);
      });
    });
  }
  ...
  /* ── Helper for "Deep" Smart Mock text ─────────────────── */
  function getSmartMockContent(fileName) {
    const name = fileName.toLowerCase();
    let content = `[DEEP ANALYSIS OF ${fileName.toUpperCase()}]\n\n`;
    
    // Core financial metrics (simulated)
    content += "### 1. FINANCIAL SUMMARY & HIGHLIGHTS\n";
    content += "Current Ratio: 2.1x (Improved from 1.8x YoY). Operating Cash Flow: ₹12,450 Cr (+14% Growth).\n";
    content += "Capex allocations for FY25 planned at ₹4,000 Cr focus on AI and Green Cloud infrastructure.\n\n";

    if (name.includes('risk')) {
      content += "### 2. DETAILED RISK EXPOSURE\n";
      content += "- High exposure to semiconductor supply chain volatility in the Far East.\n";
      content += "- Regulatory audit on personal data handling in EMEA region may lead to one-time compliance costs.\n";
      content += "- Leverage (Debt-to-Equity) stands at 0.85, higher than industry average of 0.6.\n";
    } else if (name.includes('q3') || name.includes('quarter') || name.includes('result')) {
      content += "### 2. PERFORMANCE TRENDS\n";
      content += "- Quarterly revenue exceeded estimates by 3%. Subscription-based ARR growth at 21%.\n";
      content += "- Net Margins compressed by 45bps due to increased marketing spend on GenAI product launch.\n";
      content += "- Customer acquisition cost (CAC) dropped 8% indicating better conversion efficiency.\n";
    } else {
      content += "### 2. STRATEGIC POSITIONING\n";
      content += "The company occupies a dominant 'Moat' position in the domestic enterprise market. Strategic pivot toward SaaS models is yielding strong early-stage results with lower churn rates (currently <4%).\n";
    }
    
    content += "\n### 3. FORWARD GUIDANCE\n";
    content += "Management expects double-digit growth to sustain across both B2B and consumer segments. Potential M&A activity in the cybersecurity space hinted during the recent earnings call.\n";
    content += "\n---\nAnalysis generated by FinAI deep document parser engine.";
    return content;
  }
  ...
  function bindAnalysePage() {
    ...
    const handleFileSelection = (file) => {
      ...
      // Clean UI: Store text internally but don't dump in textarea
      state.isDocUploaded = true;
      state.lastAnalysisText = mockContent;
      state.lastAnalysisName = file.name;

      $('file-ready-name').textContent = file.name;
      $('file-ready-card').classList.remove('hidden');
      $('paste-text-wrapper').classList.add('hidden');
      $('input-label').textContent = 'Analysing Uploaded Document';
  ...
  function runAnalysis(docNameArg) {
    ...
    // Use stored text if a document was uploaded, otherwise use textarea
    const text = state.isDocUploaded ? state.lastAnalysisText : $('paste-text').value.trim();
    ...
    // Handle docNameArg: if it's an event (from button click) or undefined, use default
    state.lastAnalysisName   = (typeof docNameArg === 'string' && docNameArg !== '') ? docNameArg : 'Pasted Financial Text';
